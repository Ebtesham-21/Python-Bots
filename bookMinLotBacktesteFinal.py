...
# This function is kept for potential other uses, but not for main trade execution lot sizing anymore.
...
if __name__ == "__main__":
    start_datetime = datetime(2024, 12, 1) 
        ...
        for timestamp in master_time_index:
            ...
            if not global_active_trade and not global_pending_order: 
                for sym_to_check_setup in SYMBOLS_AVAILABLE_FOR_TRADE:
                    ...
                    # --- MODIFICATION START ---
                    # 📍 Step 1: Replace Dynamic Lot Size Calculation
                    lot_size_fixed_min = props_setup.get("volume_min", 0.01)  # fallback

                    # 📍 Step 2: Estimate Risk of This Trade
                    # risk_val_diff is sl_diff
                    ...
                    # Daily risk check (using estimated_risk_min_lot)
                    ...
                    # Create pending order with fixed min lot and its calculated risk
                    global_pending_order = {"symbol":sym_to_check_setup, "type":order_type_setup, 
                                            "entry_price":entry_px, "sl_price":sl_px, 
                                            "created_time":timestamp, "lot_size":lot_size_fixed_min, # Use fixed min lot
                                            "setup_bias": m5_setup_bias_setup, 
                                            "intended_risk_amount": estimated_risk_min_lot} # Use actual risk of min lot
                    
                    daily_risk_allocated_on_current_date += estimated_risk_min_lot # Allocate the actual risk
